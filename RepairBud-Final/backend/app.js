...
mongoose
  .connect(
    "mongodb+srv://max:SdHi9lnfEIKSNmXe@cluster0-9wnr1.mongodb.net/test?retryWrites=true&w=majority",
    {
...
app.post("/api/appointments", (req, res, next) => {
  ...
  // mongooseDB has a build in save function
  // (build-in query with the post model)
  appointment.save().then(createdPost => {
    res.status(201).json({
      message: "Appointment added successfully!",
      appointId: createdPost._id
    });
  });
});
